...
function findFiles(prefix) {
  ...
  for (const entry of entries) {
    const fullPath = path.join(PASTA, entry)
...
async function carregarMunicipios() {
  ...
  await streamLines(files[0], line => {
    const cols = decodeLatin1(Buffer.from(line, 'latin1')).split(';').map(c => c.replace(/^"|"$/g, '').trim())
    if (cols.length >= 2) municipios.set(cols[0], cols[1])
  })
