...
export default function CorteLaser() {
  ...
  const waLink = c.whatsapp_numero ? `https://wa.me/${c.whatsapp_numero}?text=${encodeURIComponent(c.laser_wa_mensagem || 'Olá, gostaria de um orçamento de corte a laser.')}` : WA_LINK
  ...
  return (
    <>
      ...
      <section className={styles.hero}>
        ...
        <div className={`container ${styles.heroInner}`}>
          <div className={styles.heroContent}>
            <span className={styles.heroEyebrow}>{c.laser_hero_eyebrow || 'Serviço especializado'}</span>
            <h1 className={styles.heroTitle}>{c.laser_hero_titulo || 'Corte a Laser em Chapas Metálicas'}</h1>
            <p className={styles.heroSubtitle}>{c.laser_hero_subtitulo || 'Aço inox, aço carbono, ligas de alumínio e outros metais — precisão milimétrica para chapas, peças decorativas, letras, logos e qualquer recorte personalizado.'}</p>
      ...
      <section className={`section ${styles.aplicacoesSection}`}>
        <div className="container">
          <div className={styles.sectionHead}>
            <h2 className="section-title">{c.laser_aplicacoes_titulo || 'O que cortamos a laser'}</h2>
            <p className="section-subtitle">{c.laser_aplicacoes_subtitulo || 'Da chapa industrial à peça artística — o corte a laser permite executar qualquer geometria com precisão e acabamento superior.'}</p>
          </div>
      ...
      <section className={`section ${styles.materiaisSection}`}>
        <div className="container">
          <div className={styles.sectionHead}>
            <h2 className="section-title">{c.laser_materiais_titulo || 'Materiais que cortamos'}</h2>
            <p className="section-subtitle">{c.laser_materiais_subtitulo || 'Nossa máquina 3015G foi desenvolvida para o corte de chapas metálicas de diferentes ligas e composições.'}</p>
          </div>
        ...
        <div className="container">
          <div className={styles.industriasInner}>
            <div className={styles.industriasTexto}>
              ...
              <p className={styles.industriasIntro}>{c.laser_industrias_intro || 'A tecnologia de corte a laser é amplamente utilizada em diferentes setores industriais que exigem precisão e versatilidade no processamento de chapas metálicas.'}</p>
            </div>
        ...
        <div className="container">
          <div className={styles.vantagensGrid}>
            <div className={styles.vantagensTexto}>
              ...
              <h2 className="section-title">{c.laser_vantagens_titulo || 'Tecnologia que faz diferença'}</h2>
              <p className={styles.vantagensIntro}>{c.laser_vantagens_intro || 'O corte a laser combina precisão, velocidade e flexibilidade que processos convencionais não atingem. Ideal para peças únicas ou séries, em aço inox, aço carbono, alumínio e outros metais.'}</p>
