...
// Mapeia forma_pagamento de vendas → enum de fin_lancamentos
...
export default function AdminVendas() {
  ...
  // Recebimento inline para vendas a prazo já registradas
  ...
  // ── Criar lançamento financeiro (usado no registro e na vinculação retroativa) ─
  ...
  async function registrarVenda(e: React.FormEvent) {
    ...
    if (form.recebimento === 'a_prazo' && !form.data_vencimento) {
      setMsg({ tipo: 'erro', texto: 'Informe a data de vencimento para venda a prazo.' })
      return
    }
    setSalvando(true)
  ...
  async function registrarRecebimento(v: Venda) {
    ...
    setMsg({ tipo: 'ok', texto: 'Recebimento registrado. Lançamento baixado.' })
    setRecebendoId(null)
    fetchVendas()
    setSalvando(false)
  }

  // ── Vinculação retroativa (vendas antigas sem fin_lancamento_id) ────────────
  ...
  async function vincularFinanceiro(v: Venda) {
    ...
    if (lancId) {
      ...
      setMsg({ tipo: 'ok', texto: 'Venda vinculada ao financeiro.' })
      fetchVendas()
    }
  }
  ...
  return (
    <div className={styles.wrap}>
      ...
      <form className={styles.form} onSubmit={registrarVenda}>
        ...
        {/* Campos extras apenas para a_prazo */}
          ...
          <table className={styles.tabela}>
            ...
            <tbody>
              {vendasSorted.map(v => {
                ...
                const vencido  = pendente && v.data_vencimento && v.data_vencimento < new Date().toISOString().slice(0, 10)
